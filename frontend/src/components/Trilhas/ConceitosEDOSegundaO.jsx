...
import { data7 } from "../../assets/data7";

const ConceitosEDOSegundaO = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(data7[index]);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Conceitos sobre EDO de segunda ordem</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {data7.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {data7.length} questões
          </div>
        </>
      )}
...
export default ConceitosEDOSegundaO;
