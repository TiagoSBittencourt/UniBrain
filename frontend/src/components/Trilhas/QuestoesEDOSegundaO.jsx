...
import { data8 } from "../../assets/data8";

const QuestoesEDOSegundaO = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(data8[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === data8.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(data8[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Questões sobre EDO de segunda ordem</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {data8.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {data8.length} questões
          </div>
        </>
      )}
...
export default QuestoesEDOSegundaO;
