...
import { dataP2 } from "../../assets/dataP2";

const QuestoesMedia = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(dataP2[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === dataP2.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(dataP2[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Questões sobre Média, Mediana e Moda</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {dataP2.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {dataP2.length} questões
          </div>
        </>
      )}
