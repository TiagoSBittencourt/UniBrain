...
const ConceitosSeries = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(data3[index]);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Conceitos sobre Séries</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {data3.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {data3.length} questões
          </div>
        </>
      )}
...
export default ConceitosSeries
