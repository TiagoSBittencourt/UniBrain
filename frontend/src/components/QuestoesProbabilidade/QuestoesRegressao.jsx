...
import { dataP8 } from "../../assets/dataP8";

const QuestoesRegressao = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(dataP8[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === dataP8.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(dataP8[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Questões sobre Regressão Linear e Correlação</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {dataP8.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {dataP8.length} questões
          </div>
        </>
      )}
...
export default QuestoesRegressao;
