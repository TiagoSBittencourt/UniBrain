...
import { dataP12 } from "../../assets/dataP12";

const QuestoesEstimativa = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(dataP12[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === dataP12.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(dataP12[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1>Questões sobre Métodos de Estimação Avançados</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {dataP12.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {dataP12.length} questões
          </div>
        </>
      )}
...
export default QuestoesEstimativa;
