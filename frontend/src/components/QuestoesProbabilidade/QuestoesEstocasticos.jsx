...
import { dataP10 } from "../../assets/dataP10";

const   QuestoesEstocasticos = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(dataP10[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === dataP10.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(dataP10[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Questões sobre Processos Estocásticos</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {dataP10.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {dataP10.length} questões
          </div>
        </>
      )}
...
export default QuestoesEstocasticos;
