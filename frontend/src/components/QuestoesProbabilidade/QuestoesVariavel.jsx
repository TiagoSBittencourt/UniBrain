...
import { dataP4 } from "../../assets/dataP4";

const QuestoesVariavel = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(dataP4[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === dataP4.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(dataP4[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Questões sobre Variável Aleatória</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {dataP4.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {dataP4.length} questões
          </div>
        </>
      )}
...
export default QuestoesVariavel;
