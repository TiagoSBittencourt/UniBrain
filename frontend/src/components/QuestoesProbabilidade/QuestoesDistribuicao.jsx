...
import { dataP6 } from "../../assets/dataP6";

const QuestoesDistribuicao = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(dataP6[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === dataP6.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(dataP6[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Questões sobre Distribuições Amostrais</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {dataP6.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {dataP6.length} questões
          </div>
        </>
      )}
...
export default QuestoesDistribuicao;
