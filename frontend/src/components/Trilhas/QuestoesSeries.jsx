...
import { data4 } from "../../assets/data4";

const QuestoesSeries = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(data4[index]);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1>Questões sobre Séries</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {data4.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {data4.length} questões
          </div>
        </>
      )}
