...
import { data5 } from "../../assets/data5";

const ConceitosEDOPrimeiraO = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(data5[index]);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Conceitos sobre EDO de primeira ordem</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {data5.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {data5.length} questões
          </div>
        </>
      )}
...
export default ConceitosEDOPrimeiraO;
