...
import { data12 } from "../../assets/data12";

const QuestoesLaplace = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(data12[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === data12.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(data12[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1>Questões sobre Laplace</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {data12.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {data12.length} questões
          </div>
        </>
      )}
...
export default QuestoesLaplace;
