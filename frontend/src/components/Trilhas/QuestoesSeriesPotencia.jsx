...
import { data10 } from "../../assets/data10";

const QuestoesSeriesPotencia = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(data10[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === data10.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(data10[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1>Questoões sobre Séries de Potências</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {data10.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {data10.length} questões
          </div>
        </>
      )}
...
export default QuestoesSeriesPotencia;
