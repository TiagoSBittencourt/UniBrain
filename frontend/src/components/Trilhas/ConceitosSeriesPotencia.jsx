...
import { data9 } from "../../assets/data9";

const ConceitosSeriesPotencia = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(data9[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === data9.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(data9[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Conceitos sobre Séries de Potências</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {data9.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {data9.length} questões
          </div>
        </>
      )}
...
export default ConceitosSeriesPotencia;
