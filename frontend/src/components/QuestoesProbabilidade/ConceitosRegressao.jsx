...
import { dataP7 } from "../../assets/dataP7";

const ConceitosRegressao = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(dataP7[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === dataP7.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(dataP7[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Conceitos sobre Regressão Linear e Correlação</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {dataP7.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {dataP7.length} questões
          </div>
        </>
      )}
...
export default ConceitosRegressao;
