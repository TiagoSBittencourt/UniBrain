...
import { dataP9 } from "../../assets/dataP9";

const ConceitosEstocasticos = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(dataP9[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === dataP9.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(dataP9[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Conceitos sobre Processos Estocásticos</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {dataP9.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {dataP9.length} questões
          </div>
        </>
      )}
...
export default ConceitosEstocasticos;
