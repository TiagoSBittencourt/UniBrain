...
import { data11 } from "../../assets/data11";

const ConceitosLaplace = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(data11[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === data11.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(data11[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Conceitos sobre Laplace</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {data11.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {data11.length} questões
          </div>
        </>
      )}
...
export default ConceitosLaplace;
