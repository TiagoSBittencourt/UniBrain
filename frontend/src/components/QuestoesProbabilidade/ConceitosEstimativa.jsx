...
import { dataP11 } from "../../assets/dataP11";

const ConceitosEstimativa = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(dataP11[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === dataP11.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(dataP11[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Conceitos sobre Métodos de Estimação Avançados</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {dataP11.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {dataP11.length} questões
          </div>
        </>
      )}
...
export default ConceitosEstimativa;
