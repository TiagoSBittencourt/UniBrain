...
import { dataP } from "../../assets/dataP";

const ConceitosMedia = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(dataP[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === dataP.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(dataP[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Conceitos sobre Média, Mediana e Moda</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {dataP.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {dataP.length} questões
          </div>
        </>
      )}
