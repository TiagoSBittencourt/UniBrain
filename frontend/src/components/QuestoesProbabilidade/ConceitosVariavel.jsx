...
import { dataP3 } from "../../assets/dataP3";

const ConceitosVariavel = () => {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(dataP3[index]);
  ...
  const next = () => {
    if (lock === true) {
      if (index === dataP3.length - 1) {
        setResult(true);
        return 0;
      }
      setIndex(++index);
      setQuestion(dataP3[index]);
      setLock(false);
  ...
  return (
    <div className="containerConceitosSequencias">
      <h1> Conceitos sobre Variável Aleatória</h1>
      <hr />
      {result ? (
        <>
          <h2 className="resultado">
            Você acertou {score} de {dataP3.length} questões
          </h2>
        </>
      ) : (
        <>
          ...
          <div className="indexConceitosSequencias">
            {index + 1} de {dataP3.length} questões
          </div>
        </>
      )}
...
export default ConceitosVariavel;
